...
public class QuanLyCanBo {
	...
	// phan NEW nay la chua tai lop
	public static void themCanBoMoiNew(String hoten, int age, Gender gender, String diaChi, short type, String thuocTinhThem){
		if(type == 1){ // Cong nhan
			CongNhan  cn = new CongNhan(hoten, age, gender, diaChi, Short.valueOf(thuocTinhThem));
			canBo.add(cn);
		}else if(type == 2){ // Ki Su
			KiSu ks = new KiSu(hoten, age, gender, diaChi, thuocTinhThem);
			canBo.add(ks);
		}else if(type == 3){ // Nhan Vien
			NhanVien nv = new NhanVien(hoten, age, gender, diaChi, thuocTinhThem);
			canBo.add(nv);
		}else{	
		}
	}
